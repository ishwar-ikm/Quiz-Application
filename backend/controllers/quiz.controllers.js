...
export const getCreatedQuestion = async (req, res) => {
  try {
    const userId = req.user._id;
    ...
    if (quiz.createdBy.toString() !== userId.toString()) {
      return res.status(400).json({ error: "Quiz is not created by you" });
    }

    return res.status(200).json(quiz);
  } catch (error) {
    console.log("Error in getCreatedQuestion controller:", error.message);
    res.status(500).json({ error: "Internal server error" });
  }
}